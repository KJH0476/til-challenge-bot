...
- `reaction_removed`: 채널 내의 모든 리액션 삭제 이벤트를 구독
  ...
  저는 제가 원하는 정보만 받기 위해 총 5개의 경우로 분기처리해주었습니다.
...
**채널에 메세지가 게시된 경우**(답글 X) 위와 같이 요청 데이터가 옵니다. 이때 `type` 값이 message 인것을 확인하고 `blocks -> elements -> elements -> text`로 들어가서 [TIL N월 N주차] 글이 게시되었는지 확인합니다. 이외의 경우는 모두 무시해주고 TIL 챌린지 글이 경우에만 메세지 고유값인 `ts` 값을 저장해줍니다. 이것을 저장해주는 이유는 답글이 달린 경우, 리액션 추가 및 삭제, 답글 삭제 및 편집 이벤트가 왔을 때 TIL 챌린지 글에서 발생된 것인지 확인해주기 위함입니다.
...
리액션 같은 경우에는 채널의 게시글에 리액션을 추가한 경우와 답글에 리액션을 추가한 경우에 오는 이벤트 데이터 필드가 똑같습니다. 그래서 리액션이 추가된 메세지가 채널의 게시글인지 답글인지 확인해줘야 했습니다. 다행이 리액션이 추가된 메세지의 `ts` 값이 같이 옵니다. 이를 통해 TIL 챌린지 답글이 달릴 때 저장해두었던 `ts` 값을 이벤트 요청 데이터의 `ts` 값과 비교해줌으로써 TIL 챌린지 글의 답글에 달린 리액션인지 확인한 후 추가해주었습니다.</br>
...
원래는 리액션 개수만 유지해줬으나, 유저 아이디까지 모두 유지해준 것은 [[[문제점과 개선한점]]] 에서 말한 중복요청을 처리해주기 위함입니다. 리액션이 추가된 것을 데이터에 반영하기 전에 👍 리액션을 이미 남긴 유저아이디가 있는지 검사 후 없으면 추가하고, 있으면 다시 덮어씌여지게 해주었습니다.
